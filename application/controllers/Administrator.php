...
class Administrator extends CI_Controller {
		...
		if (isset($_POST['submit'])){
            ...
            if ($hasil['file_name']==''){
            	$data = array('nama_website'=>$this->db->escape_str($this->input->post('a')),
                                ...
                                'fb_app_id'=>$this->db->escape_str($this->input->post('fb_app_id')),
	...
	function tambah_halamanbaru(){
		...
		if (isset($_POST['submit'])){
            ...
            $config['allowed_types'] = 'gif|jpg|png|jpeg|jpg|JPG|JPEG';
 ...
 //        }
    ...
    function tambah_iklanatas(){
        ...
        if (isset($_POST['submit'])){
            ...
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG|swf|jpeg';
            $config['max_size'] = '3000'; // kb
            $this->load->library('upload', $config);
            $this->upload->do_upload('c');
            $hasil=$this->upload->data();
	...
	function edit_download(){
		...
		if (isset($_POST['submit'])){
			$config['upload_path'] = 'asset/files/';
            $config['allowed_types'] = 'gif|jpg|jpeg|png|zip|rar|pdf|doc|docx|ppt|pptx|xls|xlsx|txt';
            $config['max_size'] = '25000'; // kb
            $this->load->library('upload', $config);
            $this->upload->do_upload('b');
            $hasil=$this->upload->data();
 ...
 //            $this->email->from('admin@esida.id', 'ESIDA.ID');
 //            $this->email->to($email);
 //            $this->email->cc('');
 //            $this->email->bcc('');
